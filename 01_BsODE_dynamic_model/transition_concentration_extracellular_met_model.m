function transition_concentration_extracellular_met_model
global tdat Ydat
...
function dC5=DifEq5(t,Y5)
            dc5dt=zeros(6,1);
            u= (0.2454*Y5(2))./(Y5(2)+ k(1)); % Monod Equation
            ...
            p4= (k(8)*((Y5(5)-47.47))./(((Y5(5)-47.47)) + (k(9))))*(47.042/4); % Levan degradation
           

    dc5dt(1) = (u-0.01)*Y5(1); % Biomass Growth with death constant
    dc5dt(2) = -((((u)*Y5(1))/k(10)))-(((0.4*p2)))-(((p3)*Y5(1))); % Sucrose cons
    dc5dt(3) = (((p2*rectangularPulse(0,6,t))))+((p3)*Y5(1)); % Glucose pro
    dc5dt(4) = (((p2*rectangularPulse(0,6,t))))-((p3)*Y5(1))+((p4)*heaviside(t-24)); %Fructose prod
    dc5dt(5) = ((p3)*Y5(1))-(p4*heaviside(t-24)); % Levan Production
...
% Import dataset
data = xlsread('transition_concentration_extracellular_met.xlsx',1);
tdat = data(:,1);
